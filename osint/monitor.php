...
if ($action === 'toggle_handle') {
    scan_handle_set_enabled((int) $u['id'], ($_POST['on'] ?? '') === '1');
    echo json_encode(['ok' => true]); exit;
}
if ($action === 'dismiss') {
...
if ($action === 'dismiss_handle') {
    scan_handle_clear_pending((int) $u['id']);
    echo json_encode(['ok' => true]); exit;
}
...
if ($action === 'check') {
    ...
    $hn = scan_handle_get((int) $u['id']);
    if (!$m['enabled'] && !$ct['enabled'] && !$hn['enabled']) { echo json_encode(['ok' => true, 'ran' => false]); exit; }
    if ($m['last_check'] > 0 && (time() - $m['last_check']) < OSINT_MONITOR_INTERVAL) {
        echo json_encode(['ok' => true, 'ran' => false, 'pending' => $m['pending'], 'ct_pending' => $ct['pending'], 'handle_pending' => $hn['pending']]); exit;
    }
    $new   = $m['enabled'] ? scan_monitor_run((int) $u['id']) : 0;
    $ctNew = $ct['enabled'] ? scan_ct_run((int) $u['id']) : 0;
    $hnNew = $hn['enabled'] ? scan_handle_run((int) $u['id']) : 0;
    // Make sure the shared sweep timestamp advances even if only CT/handle ran.
    ...
    echo json_encode(['ok' => true, 'ran' => true, 'new' => $new, 'pending' => scan_monitor_get((int) $u['id'])['pending'],
                      'ct_new' => $ctNew, 'ct_pending' => scan_ct_get((int) $u['id'])['pending'],
                      'handle_new' => $hnNew, 'handle_pending' => scan_handle_get((int) $u['id'])['pending']]); exit;
}
